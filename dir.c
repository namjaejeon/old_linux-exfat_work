...
static int exfat_readdir(struct inode *inode, struct exfat_dir_entry *dir_entry)
{
	...
	if (ei->entry == -1) {
		...
		dir.size = i_size_read(inode) >> sbi->cluster_size_bits;
		dir.flags = ei->flags;
	}
	...
	if (clu.flags == 0x03) {
		...
		/* hint_information */
		if ((clu_offset > 0) && ((ei->hint_bmap.off != CLUS_EOF) &&
			(ei->hint_bmap.off > 0)) &&
	...
	while (!IS_CLUS_EOF(clu.dir)) {
		...
		for ( ; i < dentries_per_clu; i++, dentry++) {
			...
			*(uni_name.name) = 0x0;
			...
			if (!ep)
				return -EIO;
			dir_entry->size = le64_to_cpu(ep->dentry.stream.valid_size);
			brelse(bh);
...
static int exfat_iterate(struct file *filp, struct dir_context *ctx)
{
	...
	if (err)
		goto out;
get_new:
	ei->rwoffset = cpos >> DENTRY_SIZE_BITS;
	...
	cpos = ei->rwoffset << DENTRY_SIZE_BITS;
...
static void exfat_set_entry_type(struct exfat_dentry *ep, unsigned int type)
{
	if (type == TYPE_UNUSED) {
		...
		ep->type = EXFAT_FILE;
		ep->dentry.file.attr = ATTR_SUBDIR_LE;
	} else if (type == TYPE_FILE) {
		ep->type = EXFAT_FILE;
		ep->dentry.file.attr = ATTR_ARCHIVE_LE;
	} else if (type == TYPE_SYMLINK) {
		ep->type = EXFAT_FILE;
		ep->dentry.file.attr = ATTR_ARCHIVE_LE | ATTR_SYMLINK_LE;
	}
}
		...
		struct exfat_dentry *ep, unsigned int type)
{
	...
	ep->dentry.file.access_time_ms = 0;
}
		...
		for (; i < dentries_per_clu; i++, dentry++) {
			...
			if (entry_type == TYPE_STREAM) {
				...
				if ((p_uniname->name_hash ==
					le16_to_cpu(ep->dentry.stream.name_hash)) &&
					(p_uniname->name_len ==
